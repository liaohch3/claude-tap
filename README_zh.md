...
拦截并查看 Claude Code、Codex CLI、Gemini CLI、OpenCode、Pi、Kimi CLI、iFlow CLI、Cursor Agent、Qoder CLI、Devin CLI 或 Hermes Agent 的 API 流量。看清它们如何构造 system prompt、管理对话历史、选择工具、优化 token 用量——通过一个美观的 trace 查看器。
...
| 客户端 | 安装命令 | `--tap-client` |
|--------|----------|----------------|
| [Claude Code](https://docs.anthropic.com/en/docs/claude-code) | `curl -fsSL https://claude.ai/install.sh \| bash` | `claude`（默认） |
...
| [Devin CLI](https://cli.devin.ai) | 按文档链接中的安装脚本操作 | `devin` |
...
Gemini CLI 默认使用 **forward proxy** 模式，因为 Google 账号、Gemini API key 和 Vertex AI 认证可能走不同上游。API key 或 Vertex 场景也可以显式使用 reverse 模式；此时会同时设置 `GOOGLE_GEMINI_BASE_URL` 和 `GOOGLE_VERTEX_BASE_URL`。

```bash
# forward proxy 模式 — 推荐默认
claude-tap --tap-client gemini -- -p "hello"

# Gemini API key 认证的 reverse 模式
claude-tap --tap-client gemini --tap-proxy-mode reverse

# Vertex AI 的 reverse 模式：显式指定上游
...
Pi 是多 provider coding agent。claude-tap 默认使用 **forward proxy** 模式，这样不需要假设用户配置的是哪个 provider 或哪个 base URL。
...
# reverse 模式仅适合 OpenAI 兼容 provider 配置
...
<details>
<summary>iFlow CLI 示例</summary>

iFlow CLI 默认通过 `IFLOW_baseUrl` 和 `IFLOW_BASE_URL` 使用 reverse proxy。默认上游是 OpenAI 兼容的 iFlow API。
...
Qoder CLI 默认使用 **forward proxy** 模式，因为它的账号型 CLI 没有公开文档说明 provider base URL 覆盖方式。
...
<details>
<summary>Devin CLI 示例</summary>

Devin CLI 默认使用 **forward proxy** 模式。它使用 Devin 云端集成，claude-tap 会通过注入 proxy 和 CA 环境变量捕获流量。
...
```
--tap-client CLIENT      启动的客户端: claude（默认）/ codex / gemini / opencode / pi / kimi / iflow / cursor / qoder / devin / hermes
...
--tap-proxy-mode MODE    代理模式: reverse 或 forward（默认：claude/codex/kimi/iflow 用 reverse，gemini/opencode/pi/hermes/cursor/qoder/devin 用 forward）
```
