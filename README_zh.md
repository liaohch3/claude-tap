...
Codex App 会通过 claude-tap 的 forward proxy 启动，因此最终发往后端的 HTTP 和 WebSocket 请求体会像其他客户端一样进入 trace viewer。在 macOS 上，必要时 claude-tap 会把本地 CA 信任到当前用户的登录钥匙串中，让内置 app-server 能通过代理连接。
