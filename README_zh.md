...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=claude-tap)** 兼容 OpenAI API，支持 59+ LLM（Claude、GPT、Gemini、DeepSeek、Qwen 等）——可作为 claude-tap 的 drop-in API provider，trace 并检查任意模型的请求，开发者工具可申请 API credits 合作 — [coding plan](https://www.atlascloud.ai/console/coding-plan)
