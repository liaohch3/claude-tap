...
def _extract_metadata(record_json: str) -> dict | None:
    ...
    return {
        ...
        "request_generate": body.get("generate") if isinstance(body.get("generate"), bool) else None,
        "response_generate": resp_body.get("generate") if isinstance(resp_body.get("generate"), bool) else None,
        "response_output_count": len(resp_body.get("output")) if isinstance(resp_body.get("output"), list) else 0,
