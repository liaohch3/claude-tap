...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    tap_parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  # One-shot TUI — forward proxy captures whatever provider hermes is configured for\n"
            "  claude-tap --tap-client hermes\n"
            "  # Gateway mode — claude-tap auto-rewrites `gateway start` to `gateway run`\n"
            "  # so the gateway runs in foreground and inherits the proxy env.\n"
